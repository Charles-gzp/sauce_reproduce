...
class ViTSAERunnerConfig():
    ...
    sae_target_token: str = "class"  # one of: class, last
    image_caption_prompt: str = ""
    image_width: int = 224
    image_height: int = 224
    model_name: str = "openai/clip-vit-base-patch32"
    # Metadata field for SAUCE-style reproduction bookkeeping.
    # Current training path in this repo is CLIP-vision based.
    vlm_family: str = "clip"  # one of: clip, llava, llama_vision
    ...
    # Training Parameters
    l1_coefficient: float = 1e-3
    reconstruction_loss_type: str = "normalized_l2"  # one of: normalized_l2, l2
    lr: float = 3e-4
    lr_scheduler_name: str = "constant"  # constant, constantwithwarmup, linearwarmupdecay, cosineannealing, cosineannealingwarmup
    lr_warm_up_steps: int = 500
    batch_size: int = 4096
    ...
    def __post_init__(self):
        ...
        if self.vlm_family not in ["clip", "llava", "llama_vision"]:
            raise ValueError(
                f"vlm_family must be clip, llava, or llama_vision. Got {self.vlm_family}"
            )
        if self.sae_target_token not in ["class", "last"]:
            raise ValueError(
