...
class SparseAutoencoder(HookedRootModule):
    ...
    def forward(self, x, dead_neuron_mask = None):
        ...
        if self.cfg.reconstruction_loss_type == "normalized_l2":
            ...
            mse_loss = reconstruction_per_token / token_norm
        elif self.cfg.reconstruction_loss_type == "l2":
            mse_loss = reconstruction_per_token
        else:
            raise ValueError(
                f"Unsupported reconstruction_loss_type: {self.cfg.reconstruction_loss_type}"
            )
        ...
        if self.cfg.use_ghost_grads and self.training and dead_neuron_mask.sum() > 0:
            ...
            # 3. 
            ghost_recon = torch.pow((ghost_out - residual.detach().float()), 2).sum(dim=-1)
            residual_norm = residual.detach().float().pow(2).sum(dim=-1).sqrt().clamp_min(1e-8)
            mse_loss_ghost_resid = ghost_recon / residual_norm
