...
def run_evals(sparse_autoencoder: SparseAutoencoder, activation_store: ViTActivationsStore, model: HookedVisionTransformer, n_training_steps: int):
    # 当前评估依赖 CLIP 的对比损失；LLaVA 路径先跳过该指标。
    if sparse_autoencoder.cfg.vlm_family != "clip":
        return

    token_index = get_token_index(
        class_token=sparse_autoencoder.cfg.class_token,
        sae_target_token=sparse_autoencoder.cfg.sae_target_token,
    )

    def zero_ablation(activations):
        activations[:, token_index, :] = torch.zeros_like(activations[:, token_index, :]).to(activations.device)
        return (activations,) # activations of size [batch, token, dimension]
    
    def sae_hook(activations):
        activations[:, token_index, :] = sparse_autoencoder(activations[:, token_index, :])[0]
        return (activations,)
